...
data point to be dropped. After the data is dropped, all the data points to the right of
the dropped data are shifted one place to the left, and the last data point is replaced
with some random value. It is guaranteed that this random value will not be equal to the
dropped value.
...
The second data point from sensor 2 is dropped, and the last value of sensor 1 is
replaced by a 5.
    ...
    def badSensor(self, sensor1: List[int], sensor2: List[int]) -> int:
        for i in range(len(sensor1)):
            if sensor1[i] != sensor2[i]:
                # faulty sensor
                faulty_sensor = -1  # indeterminate
                if i >= len(sensor1) - 1:
                    return faulty_sensor
                # check for faulty sensor 1
                if sensor1[i:-1] == sensor2[i + 1 :]:
                    faulty_sensor = 1
                # check for faulty sensor 2
                if sensor2[i:-1] == sensor1[i + 1 :]:
                    faulty_sensor = 2 if faulty_sensor == -1 else -1
                return faulty_sensor
        return -1  # no error
