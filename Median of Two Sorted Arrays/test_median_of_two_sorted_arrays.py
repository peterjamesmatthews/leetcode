...
def test_fuzzy(s: Solution):
    ...
    for _ in range(int(1e2)):
        nums = sorted(
            [randint(int(-1e6), int(1e6)) for _ in range(randint(1, int(2e4)))]
        )

        median = (
            (nums[(len(nums) // 2) - 1] + nums[len(nums) // 2]) / 2
            if len(nums) % 2 == 0  # len(nums) is even
            else nums[len(nums) // 2]
        )

        split = randint(0, len(nums))
        nums1 = sorted(nums[:split])
        nums2 = sorted(nums[split:])

        assert s.findMedianSortedArrays(nums1, nums2) == median


def test_failure(s: Solution):
    nums1 = [
        76,
        89,
        104,
    ...
    assert s.findMedianSortedArrays(nums1, nums2)
